...
regexFailed :: (Monad m,MonadFail m) => m b
{-# INLINE regexFailed #-}
regexFailed =  Fail.fail $ "regex failed to match"

actOn :: (RegexLike r s,Monad m,MonadFail m) => ((s,MatchText s,s)->t) -> r -> s -> m t
{-# INLINE actOn #-}
...
polymatchM :: (RegexLike a b,Monad m,MonadFail m) => a -> b -> m b
{-# INLINE polymatchM #-}
